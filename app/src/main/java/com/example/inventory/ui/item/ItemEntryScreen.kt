...
fun ItemInputForm(
    ...
    Column(
        ...
        var selectedUnit by remember { mutableStateOf(ItemUnit.KG) }
        var expanded by remember { mutableStateOf(false) }  // Controls the dropdown visibility

        OutlinedTextField(
            value = itemDetails.quantity,
            onValueChange = { onValueChange(itemDetails.copy(quantity = it, unit = selectedUnit)) },
        ...
        DropdownMenu(
            ...
            ItemUnit.entries.forEach { unit ->
                DropdownMenuItem(
                    text = { Text(unit.unit) },
                    onClick = {
                        onValueChange(itemDetails.copy(unit = unit))
